...
module.exports = class SparkBot {
    ...
    resetWebhooks( callback ) {
        const Spark = require( 'ciscospark' ).init( {
            credentials: {
                authorization: {
                    access_token: this._botConfig.sparkToken
                }
            }
        } );
        Spark.webhooks.list().then( function ( list ) {
            for ( var i = 0; i < list.items.length; i++ ) {
                Spark.webhooks.remove( list.items[ i ] ).then( function ( res ) {
